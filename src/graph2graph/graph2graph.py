    ...
    def create_graph_from_lidar(lidar_scan, wind_features):
        ...
        for i in range(N):
            ...
            if collision_distances != 1:  # building scanned
                posn = LidarGraphConverter.polar_to_cartesian(
                    collision_distances, angles[i]
                )
                ...
                if connect:
                    ...
                    edge_attr.append(
                        torch.unsqueeze(torch.linalg.norm(diff, dim=0, ord=2), 0)
                    )
                    edge_attr.append(
                        torch.unsqueeze(torch.linalg.norm(diff, dim=0, ord=2), 0)
                    )
...
class Graph2GraphModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        graph = self.graph_converter.create_graph_from_lidar(
            lidar_scan[0], wind_features
        )
